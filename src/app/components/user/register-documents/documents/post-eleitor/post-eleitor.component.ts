...
export class PostEleitorComponent implements OnInit {
  ...
  post() {
    ...
    this.http.post(`${API.baseUrl}/documentos/eleitor`, this.eleitor.value, { headers: headers, observe: "response", responseType: "text" })
      .subscribe({
        next: (response) => {
          ...
          if (this.photo1 == null && this.photo2 == null) {
            ...
            this.http.post(`${API.baseUrl}/documentos/eleitor/img/${this.documentId}`, formData, { headers })
              .subscribe({
                next: () => {
                  this.toast.info('sucesso')
                  this.router.navigate(['cadastrar'])
